...
## Problem
The dashboard is showing a 500 Internal Server Error with the error:
```
code: '42703',
message: 'column c.name does not exist'
```

The `get_user_dashboard` database function is trying to query `c.name` (courses table), but the courses table uses `title` instead of `name`. Additionally, the function was also trying to reference `u.full_name`, which doesn't exist in the database. The `users` table has `first_name` and `last_name` columns instead.

## Solution
Run the following SQL in your Supabase SQL Editor to update the function:
...
BEGIN
  ...
  -- FIXED: Changed from u.full_name to construct name from first_name and last_name
  SELECT COALESCE(jsonb_agg(
    jsonb_build_object(
      'id', l.id,
      'title', l.title,
...
## What This Fixes
- **Courses table column**: Changed all references from `c.name` to `c.title` to match the actual courses table schema
  - Line 104: enrollments query - `'course_name', c.title`
  - Line 149: upcoming sessions query - `'course_name', c.title`
  - Line 177: pending assignments query - `'course_name', c.title`
  - Line 240: recent activity query - `'course_name', c.title`
- **Users table columns**: Changed from `u.full_name` to a CASE statement that constructs the instructor name from `first_name` and `last_name`
  - Lines 152-158: Constructs instructor name with fallback to email
- This matches the actual database schema where courses have `title` (not `name`) and users have separate `first_name` and `last_name` columns

## Verification
After running the script:
1. Refresh your dashboard at `http://localhost:3000/dashboard`
2. The error should be gone and the dashboard should load
3. Check the browser console - there should be no more 500 errors from `/api/user/dashboard`

## Files Modified
- `supabase/migrations/20250118_user_dashboard_function.sql` - Updated for future reference
- `src/app/(user)/dashboard/page.tsx` - Fixed client-side `full_name` query
