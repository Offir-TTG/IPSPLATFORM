-- Drop ALL versions of queue_triggered_email by specifying each signature explicitly

-- Drop the old version with parameters in different order
DROP FUNCTION IF EXISTS queue_triggered_email(
  UUID, UUID, TEXT, TEXT, UUID, TEXT, TEXT, JSONB, TIMESTAMP WITH TIME ZONE, TEXT
);

-- Drop the new version with different parameter order
DROP FUNCTION IF EXISTS queue_triggered_email(
  UUID, UUID, TEXT, TEXT, TEXT, TEXT, UUID, JSONB, TEXT, TIMESTAMP WITH TIME ZONE
);

-- Create the canonical version matching TypeScript code parameter order
