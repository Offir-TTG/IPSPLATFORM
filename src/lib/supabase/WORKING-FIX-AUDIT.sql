-- ============================================================================
-- WORKING FIX FOR AUDIT LOGGING - Uses gen_random_uuid()
-- This uses PostgreSQL's built-in gen_random_uuid() which requires no extensions
-- ============================================================================

-- Step 1: Test that gen_random_uuid() works (it should!)
DO $$
DECLARE
  test_uuid UUID;
BEGIN
  test_uuid := gen_random_uuid();
  RAISE NOTICE 'gen_random_uuid() is working: %', test_uuid;
END $$;

-- Step 2: Drop existing function completely (all versions)
...
-- Step 3: Create function using gen_random_uuid() instead
...
-- Step 6: Fix audit_events table to use gen_random_uuid() for default ID
...
BEGIN
  ...
  RAISE NOTICE 'Run: Ctrl+C then npm run dev';
