-- ============================================================================
-- FIX USER VERIFICATION AND DIAGNOSE SIGNUP ISSUES
-- ============================================================================
-- This script:
-- 1. Verifies the user exists in all required tables (auth.users, users, tenant_users, tenants)
-- 2. Fixes email_confirmed_at in auth.users
-- 3. Ensures email_verified is true in tenants table
-- 4. Provides comprehensive diagnostic output
-- ============================================================================

-- IMPORTANT: Replace 'offir.omer@gmail.com' with your actual email
DO $$
DECLARE
...
BEGIN
  RAISE NOTICE '';
  RAISE NOTICE '════════════════════════════════════════════════════════════════════════';
  RAISE NOTICE '   USER VERIFICATION & FIX SCRIPT';
  RAISE NOTICE '════════════════════════════════════════════════════════════════════════';
  ...
  ELSE
    ...
    IF v_auth_confirmed_at IS NULL THEN
      ...
      -- FIX: Confirm email in Supabase Auth
  ...
  -- ============================================================================
  -- STEP 2: CHECK USERS TABLE (USER PROFILE)
  -- ============================================================================
  RAISE NOTICE '──────────────────────────────────────────────────────────────────────';
  ...
  -- ============================================================================
  -- STEP 3: CHECK TENANTS TABLE (ORGANIZATION)
  -- ============================================================================
  RAISE NOTICE '──────────────────────────────────────────────────────────────────────';
  ...
  IF v_profile_tenant_id IS NOT NULL THEN
    ...
    IF NOT v_tenant_exists THEN
      RAISE NOTICE '❌ NOT FOUND: Tenant missing (tenant_id in profile points to non-existent tenant)';
    ELSE
      ...
      IF v_tenant_creation_method = 'self_service' THEN
        IF NOT v_tenant_email_verified THEN
          ...
          -- FIX: Mark tenant email as verified
          UPDATE tenants
          SET
            email_verified = true,
  ...
  -- ============================================================================
  -- STEP 4: CHECK TENANT_USERS TABLE (JUNCTION TABLE)
  -- ============================================================================
  RAISE NOTICE '──────────────────────────────────────────────────────────────────────';
  ...
  IF v_profile_tenant_id IS NOT NULL THEN
    ...
    IF NOT v_tenant_user_exists THEN
      ...
      -- For self-service signups, the creator should be 'owner'
  ...
  RAISE NOTICE '   └─ Tenant User (tenant_users): %', CASE WHEN v_tenant_user_exists THEN '✅ EXISTS' ELSE '⚠️  MISSING (minor)' END;
  RAISE NOTICE '';

  IF v_auth_user_id IS NOT NULL AND v_profile_exists AND v_tenant_exists THEN
  ...
  ELSE
    ...
    RAISE NOTICE '';
    RAISE NOTICE '⚠️  The signup process did not complete successfully.';
    RAISE NOTICE '   Please try signing up again or contact support.';
  END IF;
