-- ============================================================================
-- FORCE FIX FOR log_audit_event FUNCTION
-- This script will forcefully drop ALL versions of the function and recreate it
-- ============================================================================

-- 1. Enable UUID extension (must be first)
CREATE EXTENSION IF NOT EXISTS "uuid-ossp";

-- 2. Drop ALL versions of log_audit_event function (regardless of signature)
DROP FUNCTION IF EXISTS log_audit_event CASCADE;
DROP FUNCTION IF EXISTS public.log_audit_event CASCADE;

-- Also try dropping with explicit signatures (in case previous attempts left orphaned versions)
DROP FUNCTION IF EXISTS log_audit_event(UUID, TEXT, TEXT, TEXT) CASCADE;
DROP FUNCTION IF EXISTS log_audit_event(UUID, TEXT, TEXT, TEXT, TEXT) CASCADE;
DROP FUNCTION IF EXISTS log_audit_event(UUID, TEXT, TEXT, TEXT, TEXT, TEXT) CASCADE;
DROP FUNCTION IF EXISTS log_audit_event(UUID, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, JSONB, JSONB, TEXT, INET, TEXT, TEXT, TEXT, JSONB) CASCADE;

-- 3. Create the function with SECURITY DEFINER
...
BEGIN
  ...
  -- Generate correlation ID (uses uuid_generate_v4 from uuid-ossp extension)
...
-- 5. Ensure RLS policies allow inserts
...
-- ============================================================================
-- VERIFICATION QUERY
-- Run this after to verify the function was created correctly:
-- ============================================================================
-- SELECT
--   routine_name,
--   routine_type,
...
--
-- Expected result: security_type should be 'DEFINER'
-- ============================================================================
