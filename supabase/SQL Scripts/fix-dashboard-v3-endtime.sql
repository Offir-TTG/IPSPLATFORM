-- Fix V3 function to remove end_time reference (column doesn't exist)
-- Calculate end_time from start_time + duration instead
    ...
    'upcoming_sessions', (
      ...
      FROM (
        SELECT jsonb_build_object(
          ...
          'end_time', (l.start_time::timestamp + (l.duration || ' minutes')::interval),
...
COMMENT ON FUNCTION public.get_user_dashboard_v3(UUID) IS 'Dashboard aggregation V3 - Fixed to calculate end_time from start_time + duration';

SELECT 'SUCCESS: get_user_dashboard_v3 fixed and recreated!' as status;
