-- Fix get_user_dashboard_v3 to calculate end_time from start_time + duration
-- The lessons table doesn't have an end_time column, so we need to calculate it
    ...
    'enrollments', (
      ...
      FROM (
        ...
        WHERE e.user_id = p_user_id
          ...
          AND (c.is_active = true OR c.is_active IS NULL)  -- Only show active courses
    ...
    'upcoming_sessions', (
      ...
      FROM (
        SELECT jsonb_build_object(
          ...
          'end_time', (l.start_time + (l.duration || ' minutes')::interval),  -- Calculate end_time from start_time + duration
        ...
        WHERE l.tenant_id = v_tenant_id
          ...
          AND l.is_published = true  -- Only show published lessons
          AND m.is_published = true  -- Only show lessons from published modules
          AND c.is_active = true     -- Only show lessons from active courses
        LIMIT 5
      ) sessions_subquery
    ),
...
-- Add comment
COMMENT ON FUNCTION public.get_user_dashboard_v3(UUID) IS 'Dashboard aggregation V3 - Filters by publish status and calculates end_time from start_time + duration';
