-- Nuclear option: Drop all versions of the function from all schemas
DROP FUNCTION IF EXISTS public.get_user_dashboard(UUID) CASCADE;
DROP FUNCTION IF EXISTS get_user_dashboard(UUID) CASCADE;

-- Wait a moment for connections to clear
SELECT pg_sleep(1);

-- Recreate with correct schema in public schema explicitly
...
BEGIN
  ...
  -- Calculate stats - ALL SUBQUERIES to avoid GROUP BY
