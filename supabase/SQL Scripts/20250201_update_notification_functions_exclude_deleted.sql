-- ============================================================================
-- UPDATE NOTIFICATION FUNCTIONS TO EXCLUDE DELETED NOTIFICATIONS
-- ============================================================================
-- Date: 2025-02-01
-- Purpose: Update get_user_notifications and get_user_unread_count to filter
--          out soft-deleted notifications (is_deleted = true)
-- ============================================================================

-- Update get_user_notifications to exclude deleted notifications
...
BEGIN
  ...
  SELECT DISTINCT
    ...
    n.created_at,
    n.updated_at,
    (nr.id IS NOT NULL AND nr.read_at IS NOT NULL) as is_read,
    nr.read_at
  ...
  WHERE
    ...
    AND (nr.id IS NULL OR nr.is_deleted = FALSE OR nr.is_deleted IS NULL)
    AND (
...
-- Update get_user_unread_count to exclude deleted notifications
...
BEGIN
  ...
  WHERE
    ...
    -- Exclude deleted notifications
    AND (nr.id IS NULL OR nr.is_deleted = FALSE OR nr.is_deleted IS NULL)
    -- Unread means no read record OR read record with read_at NULL
