-- Fix dashboard stats to correctly count all courses (standalone + program courses)
-- This fixes:
-- 1. total_courses - was only counting standalone courses, missing program courses
-- 2. Stats now properly count courses from both product types
...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    'stats', jsonb_build_object(
      'total_courses', (
        -- FIXED: Count courses from BOTH standalone products AND program products
...
-- Add comment
COMMENT ON FUNCTION public.get_user_dashboard_v3(UUID) IS 'Dashboard aggregation V3 - Fixed to count all courses (standalone + program)';
