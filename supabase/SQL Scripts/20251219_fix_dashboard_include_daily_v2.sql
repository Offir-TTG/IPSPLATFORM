...
-- Also fix enrollments to use product_id instead of non-existent program_id/course_id
  ...
  SELECT jsonb_build_object(
    ...
    'upcoming_sessions', (
      ...
      FROM (
        ...
        JOIN enrollments e ON e.user_id = p_user_id AND e.status = 'active'
...
-- Add comment
COMMENT ON FUNCTION public.get_user_dashboard_v3(UUID) IS 'Dashboard aggregation V3 - Includes both Zoom and Daily.co sessions, uses product-based enrollments';
