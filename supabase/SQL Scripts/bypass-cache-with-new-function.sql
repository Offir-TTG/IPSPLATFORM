-- Bypass Supabase connection pooler cache by creating a new function with a different name
-- This forces PgBouncer to create a new cache entry instead of using the cached old one

-- Create the new function with name get_user_dashboard_v2
CREATE OR REPLACE FUNCTION public.get_user_dashboard_v2(p_user_id UUID)
RETURNS JSONB
LANGUAGE plpgsql
...
GRANT EXECUTE ON FUNCTION public.get_user_dashboard_v2(UUID) TO authenticated;

-- Add a comment
COMMENT ON FUNCTION public.get_user_dashboard_v2(UUID) IS 'Dashboard aggregation - V2 to bypass connection pooler cache';

-- Test call
SELECT 'Function get_user_dashboard_v2 created successfully!' as status;
