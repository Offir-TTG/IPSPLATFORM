...
-- 1. Attendance rate should count 'late' and 'excused' as attended
-- 2. Program enrollments should show total lessons from all courses in program
-- 3. Ensure proper lesson counting for both course and program enrollments
...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    'stats', jsonb_build_object(
      ...
      -- FIXED: Count 'present', 'late', and 'excused' as attended
      'attendance_present', (
        SELECT COUNT(*)
        FROM attendance a
        WHERE a.student_id = p_user_id
      ...
      -- FIXED: Calculate attendance rate including late and excused
      'attendance_rate', (
        SELECT CASE
          WHEN COUNT(*) > 0 THEN
...
-- Add comment
COMMENT ON FUNCTION public.get_user_dashboard_v3(UUID) IS 'Dashboard V3 - Fixed attendance calculation and program lesson counting';
