        ...
        SELECT jsonb_build_object(
          ...
          'total_lessons', COALESCE(
            (
              ...
              WHERE prod.course_id IS NOT NULL
                AND m.course_id = prod.course_id
            ), 0
          )
        ) as enrollment_data
        FROM enrollments e
...
COMMENT ON FUNCTION public.get_user_dashboard_v3 IS 'Get user dashboard data - enrollments only have product_id, join through products table to get program/course info';
