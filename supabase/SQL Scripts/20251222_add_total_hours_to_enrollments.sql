-- Add total_hours to enrollment data in dashboard
-- Calculates total duration of all lessons in each enrollment
...
BEGIN
  ...
  SELECT jsonb_build_object(
      ...
      FROM (
        SELECT jsonb_build_object(
          ...
          'total_lessons', COALESCE(
            (
              -- For course enrollments: count lessons in that course
              -- For program enrollments: count lessons in ALL courses in the program
              SELECT COUNT(*)
              FROM lessons l
              JOIN modules m ON l.module_id = m.id
              JOIN courses course ON m.course_id = course.id
              WHERE (
                -- Direct course enrollment
                (prod.course_id IS NOT NULL AND course.id = prod.course_id)
                OR
                -- Program enrollment - all courses in program
                (prod.program_id IS NOT NULL AND EXISTS (
                  SELECT 1 FROM program_courses pc
                  WHERE pc.program_id = prod.program_id
                    AND pc.course_id = course.id
                ))
              )
              AND l.is_published = true
              AND m.is_published = true
          ...
          'total_hours', COALESCE(
            (
              -- Calculate total duration in hours for all lessons in enrollment
              SELECT ROUND((SUM(l.duration) / 60.0)::NUMERIC, 1)
              FROM lessons l
              JOIN modules m ON l.module_id = m.id
              JOIN courses course ON m.course_id = course.id
              WHERE (
    ...
    'stats', jsonb_build_object(
      ...
      -- Calculate attendance rate including late and excused
      'attendance_rate', (
        SELECT CASE
          WHEN COUNT(*) > 0 THEN
            ROUND((COUNT(*) FILTER (WHERE status IN ('present', 'late', 'excused'))::NUMERIC / COUNT(*) * 100), 1)
          ELSE 0
        END
        FROM attendance a
        WHERE a.student_id = p_user_id
      ...
      -- Total hours from lesson durations (calculated from enrollments on frontend)
...
-- Add comment
COMMENT ON FUNCTION public.get_user_dashboard_v3(UUID) IS 'Dashboard V3 - Added total_hours calculation to enrollments';
