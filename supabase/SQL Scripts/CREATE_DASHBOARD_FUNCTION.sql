-- CORRECTED USER DASHBOARD FUNCTION V3
-- This version fixes all schema mismatches identified in the investigation:
-- 1. Uses enrollment_id correctly for user_progress joins
-- 2. Joins lessons through modules table (lessons.module_id → modules.course_id)
-- 3. Calculates end_time from start_time + duration (no end_time column exists)
-- 4. Uses time_spent_seconds (not minutes)
-- 5. Includes expires_at field for enrollments
-- 6. Uses DISTINCT for lesson counts to avoid double-counting
...
BEGIN
  ...
  -- Build complete dashboard data with corrected schema references
...
COMMENT ON FUNCTION public.get_user_dashboard_v3(UUID) IS 'Dashboard aggregation V3 - Fixed schema: uses enrollment_id, proper module joins, calculated end_time';
