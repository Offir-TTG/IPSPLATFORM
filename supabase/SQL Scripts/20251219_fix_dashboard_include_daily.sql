-- Fix get_user_dashboard_v3 to include both Zoom and Daily.co sessions
-- Currently only shows Zoom sessions, but should show both
    ...
    'upcoming_sessions', (
      ...
      FROM (
        ...
        WHERE l.tenant_id = v_tenant_id
          ...
          AND (l.zoom_meeting_id IS NOT NULL OR zs.daily_room_name IS NOT NULL)  -- Include both Zoom AND Daily.co
          AND l.is_published = true
          AND m.is_published = true
...
-- Add comment
COMMENT ON FUNCTION public.get_user_dashboard_v3(UUID) IS 'Dashboard aggregation V3 - Includes both Zoom and Daily.co sessions, filters by publish status';
