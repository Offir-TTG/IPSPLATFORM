-- Fix Keap translations - complete set with no duplicates
...
BEGIN
  -- First, delete all existing Keap translations to start fresh
  ...
  -- Now insert all translations fresh
  ...
  RAISE NOTICE 'Successfully inserted % Keap translations', 154;

EXCEPTION
  WHEN OTHERS THEN
    RAISE NOTICE 'Error inserting Keap translations: %', SQLERRM;
    RAISE;
END $$;
